...
class BaseVevTrader:
    ...
    def _build_smile_snapshot(
        ...
        if self.STRATEGY_MODE == "black_scholes":
            shared_vol = self._shared_live_vol(snapshot)
            if shared_vol is not None:
                for voucher in self.BS_TRADE_VOUCHERS:
                    snap = snapshot.get(voucher)
                    if snap is None:
                        continue
                    fair = self._bs_call_price(spot_mid, snap["strike"], snap["tte_years"], shared_vol)
                    snap["shared_vol"] = shared_vol
                    snap["fitted_iv"] = shared_vol
                    snap["fair"] = fair
                    snap["residual"] = snap["mid"] - fair
    ...
    def _bs_single_vol_target(
        ...
        residual = float(snap["mid"]) - float(snap["fair"])
        spread = float(snap["spread"])
        threshold = max(1.2, 0.55 * spread)
        if abs(residual) < threshold:
            return 0

        base = self.VOUCHER_TRADE_UNIT * (
            1 + int(max(0.0, abs(residual) - threshold) / max(1.0, spread))
        )
        if late_session:
        ...
        max_target = self._scaled_voucher_limit(float(snap["tte_years"]), late_session)
        return -self._sign(residual) * min(rounded, max_target)
    ...
    def _shared_live_vol(self, smile: Dict[str, Dict[str, float]]) -> Optional[float]:
        values = [
            float(smile[voucher]["iv"])
            for voucher in self.CORE_TRADE_VOUCHERS
            if voucher in smile and float(smile[voucher]["vega"]) >= self.VEGA_FLOOR
        ]
        if len(values) < 2:
            return None
