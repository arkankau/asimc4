...
class Trader:
    """
    v19 — v9b IPR + ACO layered passive orders.
    Instead of one resting bid/ask, post 4 levels on each side at increasing
    distance from fair value. Captures more flow when ACO makes larger moves.
    Aggressive takes identical to v10. Passive fill benefit only visible in live.
    """
    ...
    # Passive bid/ask layer offsets from skewed fair value (ticks)
    # Sizes are fractions of remaining capacity after aggressive takes
    ACO_BID_LAYERS = [(1, 0.25), (4, 0.25), (8, 0.25), (13, 0.25)]   # (offset below fair, fraction)
    ACO_ASK_LAYERS = [(1, 0.25), (4, 0.25), (8, 0.25), (13, 0.25)]   # (offset above fair, fraction)
    ...
    def _trade_aco(self, state: TradingState, memory: dict):
        ...
        # Aggressive takes — identical to v10
        for ask in sorted(od.sell_orders):
        ...
        # Layered passive bids — 4 levels below fair
        if buy_cap > 0:
            remaining = buy_cap
        ...
        # Layered passive asks — 4 levels above fair
        if sell_cap > 0:
            remaining = sell_cap
